		...
		measure_Hanlde(all_dimensionality,all_measure,null,function(data){
			...
			for(var i = 0;i < data.length;i++){
				...
				for(var j = 0;j < all_measure.length;j++){ // 计算出series
					if(!series[j]){
						series[j] = {"name":all_measure[j],"type":"bar","xAxisIndex":all_dimensionality.length-1,"data":[aData[drag_measureCalculateStyle[all_measure[j]]]]};
					}else{
	...
	function barChart_generate_fun(){
		measure_Hanlde(all_dimensionality,all_measure,null,function(data){
			...
			for(var i = 0;i < data.length;i++){
				...
				for(var j = 0;j < all_measure.length;j++){ // 计算出series
					if(!series[j]){
						series[j] = {"name":all_measure[j],"type":"bar","yAxisIndex":all_dimensionality.length-1,"data":[aData[drag_measureCalculateStyle[all_measure[j]]]]};
					}else{
						series[j]["data"].push(aData[drag_measureCalculateStyle[all_measure[j]]]);
					}
				}
