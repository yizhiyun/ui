		...
		$.ajax({
			...
			success:function(data){
				...
				if (data["status"] == "success") {
					...
					}else if(Object.getOwnPropertyNames(preClickView).length != 0 && preClickView[$("#pageDashboardModule #dashboard_content #new_view .auto_show").find(".folderview_li_span").text()]["viewtype"] != null){
...
leftBar_sizeW_function();
	...
	function empty_viem_init(change_or_click){
		...
		$("#pageDashboardModule #dashboard_content #lateral_bar .dimensionality_search").hide();
		// 移除表格的数据
		emptyAllTable();
	...
	function clickViewTo(title){
		...
		if(/-/gi.test(title)){
			$.post("../dashboard/setSwitch",{"switch":"isopen","id":$(".rightConent #dashboard_content #new_view ul li[title="+$("#pageDashboardModule #clickWallDelete").data("nowDeleteView")+"]").data("tableViewId")},function(result){
				if(result["status"] == "ok"){
					$("#pageStatementsModule .statement_li").eq(sessionStorage.getItem("edit_view_now").split(",")[4]).find(".view_show_handle").eq(sessionStorage.getItem("edit_view_now").split(",")[5]).find(".small_view_text").data("setopen",false);
				}
			});
		}
 ...
 function navDashboardEventInit(autoDrag){
	...
	function mdHandleFun(){
		...
		$("#pageDashboardModule #dashboard_content #lateral_bar .search").click(function(event){
			...
			inputSearch($(this).parent().parent().find(".dimensionality_search .dimensionality_search_input"),$(this).parent().parent().find(".viewTableShow ul li .drop_list_main .drop_main span").attr("class"),$(this).parent().parent().find(".viewTableShow ul"),"md");
