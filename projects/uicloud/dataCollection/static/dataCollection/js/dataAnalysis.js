      ...
      function alreadyChangeName(element,eleInput,eleText,postType,dbdata,dbmd){
            ...
            $(element).find("."+eleInput+"").show().val($(element).find("."+eleText+"").text()).focus().width($(element).width());
      ...
      function setTheDataSetActiveFunction(ele,sourceType){
        ...
        $(ele).siblings("li.active").removeClass("active");
          ...
          $(".boxDiv").each(function(index,ele){
            ...
            $(".mainDragArea #dragTableDetailInfo .imgBox[flag=deleteTable]").trigger("click");
          })
      }
      ...
      function expression_click_handle(){
        ...
        function ElementAutoSize(){
          ...
          $("#analysisContainer .mainDragArea").css({"margin-left":$("#analysisContainer .leftSlide").width() + "px","height":(document.offsetHeight | document.body.offsetHeight) - 50 + "px"});
        ...
        $("#analysisContainer .leftSlide").resizable({
            ...
            resize:function(event,ui){
              ...
              $("#analysisContainer .leftSlide #dataSet .detailDataSetList li .didBuildTables ul.tablesList .datebaseLise").css("width",(ui.size.width-38)+"px")
               $("#analysisContainer .leftSlide #dataSet .sqlSearch").css("width",(ui.size.width-40)+"px");
            }
          })
      ...
      //       var rs = data;
              ...
              if(data["status"] == "success" && data["tableAvailable"] == "true"){
                  init_clear();
              ...
              if(data["status"] == "success" && data["tableAvailable"] == "false"){
      ...
      $("#buildDataPanelView .build-footer .confirmBtn,#build_upload .confirmBtn").click(function(){
        ...
        function connectDataBaseViewShowFunction(dataBaseName,isEditConnect,preDBInfo){
          ...
          $("#connectDataBaseInfo .common-head img.logo").attr("src",dataBaseName.dataBaseImgSrc());
          $("#connectDataBaseInfo").show('shake',500,analysisbBaseInfoShowCallBack);
        }
      ...
      //      上面是链接数据库的字段信息
        ...
        //  连接数据库的弹框显示之后，处理里面的点击事件
          function analysisbBaseInfoShowCallBack(){
        ...
        function deleteTableHandleFunction(dbInfo){
           // 移除线
            ...
            //删除存储的关联
            for(handleKey in saveTableRelevance){
              var hangleTableData = handleKey.split("_YZYPD_");
              if($.inArray(dbInfo.split("_YZYPD_")[2],hangleTableData) != -1){
                delete saveTableRelevance[handleKey];
              }
            }
        ...
        function createTableDetailView(dbInfo,rs,isAllFields){
          ...
          // 清空
                ...
                for (var i= 0;i <  bottom_panel_fileds.length;i++) {
                  var img = $("<img/>");
                  var th = $("<th title='单击选中列' showType="+ bottom_panel_fileds[i]["type"]+" showValue="+ bottom_panel_fileds[i]["field"]+"><span>" + bottom_panel_fileds[i]["field"]+"</span></th>");
