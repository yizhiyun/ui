  ...
  //表格同比展示
  function tableChangeMeDi(element){
            ...
            setTimeout(function(){
              if(element){
                if(element.hasClass("compared")){
                    $("#text_table_need_show .content_body #data_list_for_body .measureDiv p.compareP").show();
                }else{
                    $("#text_table_need_show .content_body #data_list_for_body .measureDiv p.linkP").show();
                }
              }
               $("#text_table_need_show .measureDiv,#text_table_need_show .aMeasureAli").not($("#text_table_need_show .meaHeader")).css("padding","5px 0px");
               $("#text_table_need_show .measureDiv p").css("height","auto").css("lineHeight","normal");
              
               $("#text_table_need_show .aMeasureAli").height($("#text_table_need_show .measureDiv").not($("#text_table_need_show .meaHeader")).eq(0).actual("outerHeight") -1);
               $("#text_table_need_show .measureDiv").not($("#text_table_need_show .meaHeader")).css("marginTop","5px");
               if($("#text_table_need_show .left_row_container table thead tr th").length > 0){
                  $("#text_table_need_show .left_row_container table tbody tr td").height($("#text_table_need_show .measureDiv").not($("#text_table_need_show .meaHeader")).eq(0).actual("outerHeight") - 1);
                  $("#text_table_need_show .measureDiv").not($("#text_table_need_show .meaHeader")).each(function(index,ele){
                    ...
                    if($(".top_column_container").css("display") == "none"){
  ...
  //								    CodeMirror.velocityCustomizedKeywords = "server.ip server.cache software.conf software.version software.tags.count";
                ...
                editor.setValue("");
  					...
  					out_wrap_click.find(".compared").add(out_wrap_click.find(".linkBack")).click(function(event){
  						event.stopPropagation();
              if($(this).children("p").children("span").css("visibility") == "visible"){
                  $(".me_out_content").remove();
                  $(".me_out_content li").unbind("mouseenter mouseleave");
                   open_or_close = true;
                   return;
              }
              if(save_now_show_view_text.data("show_view_fun") && save_now_show_view_text.data("show_view_fun") == "showTable_by_dragData()"){
                    tableChangeMeDi($(this));
               }
