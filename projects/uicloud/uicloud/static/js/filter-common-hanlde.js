	...
	function editFilterViewShow_fun(from,successFun){
			...
			if(from == "buildData"){
				 ...
				 if($("#tableDataDetailListPanel .mainContent table thead tr th[isselect=true]").length > 0){
						var type = $("#tableDataDetailListPanel .mainContent table thead tr th[isselect=true]").data("type");
						didSelectColumn = $("#tableDataDetailListPanel .mainContent table thead tr th[isselect=true]").children("span").text();
			...
			for (var i= 0;i < filterRecord.length;i++) {
				...
				var li = $("<li columnValue="+aRecordIndictor["column"]+">"+aRecordIndictor["column"]+"	 选择内容:   "+aRecordIndictor["indictorText"]+"</li>");
	...
	//			userSelect_num.comboSelect();
				...
				//值范围输入
				$("#number-filter .number-filter-body  .slider-common-setting .value-input-box div.scalesInput-box input").each(function(index,ele){
					$(ele).unbind("input");
					...
					$(ele).on("focusout",function(){
						if($(ele).hasClass("min-value-input")){
							if(Number($(this).val()) < Number($("#number-filter .number-filter-body  .slider-common-setting .value-slider-box .range-flag .min-value-flag").text()) || Number($(this).val()) > Number($("#number-filter .number-filter-body  .slider-common-setting .value-slider-box .range-flag .max-value-flag").text())){
								this.value = $("#number-filter .number-filter-body  .slider-common-setting .value-slider-box .range-flag .min-value-flag").text();
							}
						}else{
							if(Number($(this).val()) > Number($("#number-filter .number-filter-body  .slider-common-setting .value-slider-box .range-flag .max-value-flag").text()) || Number($(this).val()) < Number($("#number-filter .number-filter-body  .slider-common-setting .value-slider-box .range-flag .min-value-flag").text())){
								this.value = $("#number-filter .number-filter-body  .slider-common-setting .value-slider-box .range-flag .max-value-flag").text();
							}
						}

						// 滚动条更新
						$("#number-filter  #value-range-box .value-slider-box .slider-ranage").eq(0).data("uiSlider")._setOption("values",[$("#number-filter .number-filter-body  .slider-common-setting .value-input-box div.scalesInput-box .min-value-input").val(),$("#number-filter .number-filter-body  .slider-common-setting .value-input-box div.scalesInput-box .max-value-input").val()])
	...
	function localStorageSaveData(tableInfo,type,column,state,savedIndex){
		...
		if(filterDataArr){
			if(state == "edit" && savedIndex >= 0){
				...
				if(filterDataArr.hasObject("column",column) == -1){
					filterDataArr.push(new_obj_filter);
				}else{
					filterDataArr.splice(filterDataArr.hasObject("column",column),1,new_obj_filter);
				}
	...
	function getCurrentTableFilterData(tableInfo,filterColumnArr){
		...
		for(var i = 0;i < arr.length;i++){
			...
			if(obj.type == "contentChooser"){
				...
				if(obj["topValue"] != ""){
					if(obj["topValue"] == "min"){
						var freeFilterNum = numberColumn_needValueInfo[current_cube_name][obj.column]["min"];
					}else if(obj["topValue"] == "max"){
						var freeFilterNum = numberColumn_needValueInfo[current_cube_name][obj.column]["max"];
					}else if(obj["topValue"] == "averge"){
						var freeFilterNum = numberColumn_needValueInfo[current_cube_name][obj.column]["averge"];
					}else{
						var freeFilterNum = obj["topValue"];
					}
					var filterNum = {"type":obj["topRelation"],"columnName":"`"+obj.column+"`","value":freeFilterNum}
					conditionFilter_record[tableInfo]["condition"].push(filterNum);
				}
